...
class MY_Model extends CI_Model {
	...
	public function insert($data, $skip_validation = FALSE) {
		...
		if($valid) {
			//Skip those would not update to table
	 ...
	 * @return void
	 */
	protected function _run_before_get()
	{
	 ...
	 * @return void
	 */
	protected function _run_after_get($result)
	{
...
class APP_Model extends MY_Model{
	...
	public function get_data($where,$limit='',$offset='', $order='', $direction='ASC',$table=""){
		$this->_run_before_get();
	...
	public function find_by($primary_value){
		$this->_run_before_get();
		$row = $this->db->where($this->primary_key, $primary_value)
						->get($this->_table)
						->row_array();
	...
	public function distinct($table_field){
		$this->_run_before_get();
		$this->db->distinct();
	...
	function word_truncate($string, $limit, $break=".", $pad="..."){
