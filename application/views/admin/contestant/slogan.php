<script>
  ...
  get_list(queryParam);	 
	...
	function get_list(queryParam){ 
		$.get(queryString+'/get_slogan_list/'+queryParam, function(data) {
			jQuery('#loading').hide();
	...
	function downloadSloganList(){
		window.location.href = queryString + "/downloadSloganList/"+queryParam;
		return false;
	}
	
</script>
<div class="container_header">
	<div class="header_title"><a class="separator" href="<?= $this->config->item('domain') ?>">Home</a> Slogan Report</div>
    <div class='search_panel' style="float: left;">
        <div style="float:left; padding-left:10px;">
            <form action="<?= $this->config->item('admin_url') ?>/<?= $this->name ?>/slogan" method="get">	
         ...
         <button type="button" value="Export" style="margin: 2px 15px;" onClick='return downloadSloganList()'>Export List</button>  
...
<div id="submenu">
	<ul>
      ...
      <li class="li-selected"><a href="<?php echo $this->config->item('admin_url').'/'.$this->name.'/slogan'?>">Slogan Report</a></li> 
