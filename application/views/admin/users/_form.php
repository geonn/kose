...
<table class="edit bordered">
	<tbody>	
		<tr>
			...
			<td>
				...
				<?php 
					...
					$res = $this->permissions_model->checkExists($roles, $this->name);		
					if($module == "edit") {
		...
		<tr>
			...
			<td><?= form_input('email', set_value('email',isset($form) ? $form['email'] : ''),'class="required 105 101" placeholder="Email Address" style="width:98%;"'); ?></td>			
		</tr>			
		...
		<tr>
			...
			<td>
				<?php
					if($res[0]['permission'] > 2){	 
						echo form_dropdown('status', $this->config->item('account_status'), set_value('status',isset($form) ? $form['status'] : ''), ' style="width:20%;"'); 
					}else{
